...
태그 개수 제한 경고: 프로필 수정(PUT) 시 기술 스택(검색 태그)이 **5개를 초과(6개 이상)**할 경우 서버에서 400 Bad Request 에러를 반환할 예정입니다. 사용자가 태그를 6개 이상 등록하려고 하면 프론트엔드 단에서 알림창(alert) 등으로 사전에 차단해 주시기 바랍니다. (태그 옆 X 버튼을 통한 삭제 기능 구현 필요)
